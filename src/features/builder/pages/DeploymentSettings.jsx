    ...
    <>
      ...
      <div className={`deploy-settings-layout${tailoredPages.length > 0 ? ' deploy-settings-layout--with-preview' : ''}`}>
        <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <div className="label">Generated from</div>
            <div style={{ fontWeight: 600, fontSize: 15, marginTop: 4 }}>{template?.name || templateId || 'Selected template'}</div>
            {tailoredSite?.answers?.businessName && <div className="muted" style={{ fontSize: 13, marginTop: 2 }}>Customized for {tailoredSite.answers.businessName}</div>}
          </div>

          <div>
            <div className="label">Site name *</div>
            <input className="input" value={siteName} onChange={(e) => setSiteName(e.target.value)} placeholder="My awesome store" autoFocus />
          </div>

          <div>
            <div className="label">Glondia subdomain</div>
            <input className="input" value={subdomain} onChange={(e) => setSubdomain(e.target.value)} placeholder={slugify(siteName || 'my-site')} />
            <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>Will be available at <span className="mono">{siteSlug}.glondia.app</span></div>
          </div>

          <div>
            <div className="label">Render service type</div>
            <select className="select" value={serviceType} onChange={(e) => setServiceType(e.target.value)}>
              <option value="static_site">Static site recommended</option>
          ...
          <div>
            <div className="label">Plan / billing estimate</div>
            <select className="select" value={plan} onChange={(e) => setPlan(e.target.value)}>
              <option value="starter">Starter</option>
              <option value="standard">Standard</option>
              <option value="pro">Pro</option>
            </select>
            <div className="card" style={{ padding: 12, marginTop: 10, background: 'var(--bg-deep)', fontSize: 13 }}>
              <b>{selectedPlan.label}:</b> {selectedPlan.estimate}
              <div className="muted" style={{ marginTop: 4 }}>{selectedPlan.note}</div>
              <div className="faint" style={{ marginTop: 8 }}>Billing is informational for now. Final payment enforcement will be added later.</div>
            </div>
          </div>
          ...
          <div className="row" style={{ gap: 10, justifyContent: 'flex-end' }}>
            <button className="btn btn-outline" onClick={() => navigate({ view: 'builder-ai-intake', params: { templateId, templateType } })}>← Back to RoxanneAI</button>
            ...
            <button className="btn btn-primary" onClick={handleDeploy} disabled={deploying || !siteName.trim()}><ICN.Rocket size={14} /> Deploy site</button>
          </div>
        </div>
