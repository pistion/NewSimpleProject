    ...
    <>
      ...
      <div className={`deploy-settings-layout${tailoredPages.length > 0 ? ' deploy-settings-layout--with-preview' : ''}`}>
        <div className="card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div><div className="label">Render service type</div><select className="select" value={serviceType} onChange={(e) => setServiceType(e.target.value)}><option value="static_site">Static site recommended</option><option value="web_service">Web service</option></select></div>
          ...
          <div className="card" style={{ padding: 14, background: 'var(--bg-deep)', fontSize: 13 }}>
            <div className="row between" style={{ gap: 12 }}><div><div style={{ fontWeight: 600 }}>Render source repository</div><div className="muted" style={{ marginTop: 3 }}>Optional here. If blank, backend uses <span className="mono">RENDER_GENERATED_SITES_REPO_URL</span>. If neither exists, the site is generated and saved, but Render handoff is skipped with clear logs.</div></div><button type="button" className="btn btn-sm btn-outline" onClick={() => setShowAdvanced(!showAdvanced)}>{showAdvanced ? 'Hide' : 'Configure'}</button></div>
            {showAdvanced && <div style={{ display: 'grid', gap: 12, marginTop: 14 }}><div><div className="label">Repository URL</div><input className="input" value={repoUrl} onChange={(e) => setRepoUrl(e.target.value)} placeholder="https://github.com/your-org/generated-sites" /></div><div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><div><div className="label">Branch</div><input className="input" value={branch} onChange={(e) => setBranch(e.target.value)} placeholder="main" /></div><div><div className="label">Root directory</div><input className="input" value={rootDirectory} onChange={(e) => setRootDirectory(e.target.value)} placeholder="optional/generated/site/path" /></div></div></div>}
          </div>

          <div><div className="label">Plan / billing estimate</div><select className="select" value={plan} onChange={(e) => setPlan(e.target.value)}><option value="starter">Starter</option><option value="standard">Standard</option><option value="pro">Pro</option></select><div className="card" style={{ padding: 12, marginTop: 10, background: 'var(--bg-deep)', fontSize: 13 }}><b>{selectedPlan.label}:</b> {selectedPlan.estimate}<div className="muted" style={{ marginTop: 4 }}>{selectedPlan.note}</div><div className="faint" style={{ marginTop: 8 }}>Billing is informational for now. Final payment enforcement will be added later.</div></div></div>
          ...
          <div className="row" style={{ gap: 10, justifyContent: 'flex-end' }}><button className="btn btn-outline" onClick={() => navigate({ view: 'builder-ai-intake', params: { templateId, templateType } })}>← Back to RoxanneAI</button><button className="btn btn-outline" onClick={handleOpenPreview} disabled={tailoredPages.length === 0}><ICN.ExternalLink size={14} /> Preview site</button><button className="btn btn-primary" onClick={handleDeploy} disabled={deploying || !siteName.trim()}><ICN.Rocket size={14} /> Deploy site</button></div>
        </div>
