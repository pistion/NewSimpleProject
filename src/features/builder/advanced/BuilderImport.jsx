    ...
    <div className={`bld-preview-frame import-loader-frame ${!isImporting ? 'import-loader-frame--still' : ''}`}>
      <div className="import-loader-shell">
        <div className="import-loader-copy">
          ...
          <h2>{title}</h2>
          <div className="muted">
            {zipFile ? <span className="mono">{Math.round(zipFile.size / 1024)} KB ZIP package</span> : repo ? <span className="mono">{repo.url} - {branch}</span> : activeLabel}
          </div>
        </div>
        ...
        {showLoader ? (
          ...
          <div className="import-loader-standby">
            ...
            <span>{zipFile ? 'ZIP ready' : repo ? 'Repository detected' : 'Waiting for project'}</span>
          </div>
        )}
        ...
        <div className="term import-loader-term">
          ...
          {zipFile && <div><span className="ts">zip</span> <span className="dim">{zipFile.name}</span></div>}
...
export function BuilderImport({ mode = 'github', navigate }) {
  ...
  const handleGitConnect = async () => {
    ...
    setGitBusy(true); setGitError(null); setImportPhase('pulling');
    ...
    try {
      ...
      const site = await importBuilderSiteFromGithub({
        ...
        outputDirectory,
        renderConfig: {
          provider: 'render', serviceType: renderConfig.serviceType, plan: renderConfig.plan,
          frontend: { rootDirectory: renderConfig.frontendRootDirectory, buildCommand: renderConfig.frontendBuildCommand, publishDirectory: renderConfig.frontendPublishDirectory },
          backend: { rootDirectory: renderConfig.backendRootDirectory, buildCommand: renderConfig.backendBuildCommand, startCommand: renderConfig.backendStartCommand },
      ...
      window.setTimeout(() => navigate({ view: 'builder-editor', params: { id: site.templateId || null, siteId: site.id } }), 700);
    } catch (err) {
  ...
  const handleZipDeploy = async () => {
    ...
    setZipBusy(true); setZipError(null); setImportPhase('uploading');
    clearTimeout(phaseTimer.current);
    phaseTimer.current = setTimeout(() => setImportPhase('building'), 1000);
    try {
      const result = await deployZipTemplate(zipFile, {
        siteName: zipFile.name.replace(/\.zip$/i, ''),
        slug: zipFile.name.replace(/\.zip$/i, ''),
        serviceType: renderConfig.serviceType,
        plan: renderConfig.plan,
        environment: 'production',
        buildCommand: renderConfig.frontendBuildCommand || 'npm run build',
      ...
      clearTimeout(phaseTimer.current);
      setImportPhase('complete');
      window.setTimeout(() => navigate({ view: 'hosting-detail', params: { id: result.deploymentId } }), 700);
    } catch (err) {
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <a className="page-eyebrow" href="#" onClick={(e) => { e.preventDefault(); navigate({ view: 'builder-gallery' }); }}>Back to site builder</a>
          <h1>Import your own work</h1>
          <p className="sub">Deploy from GitHub or drag-and-drop a ZIP package. ZIP upload creates a Hosting record and starts Render when provider settings are configured.</p>
        </div>
      </div>

      <div className="tabs" style={{ marginBottom: 14 }}>
        <button className={activeMode === 'github' ? 'active' : ''} onClick={() => { setActiveMode('github'); setImportPhase(repoUrl ? (detectedRepo ? 'detected' : 'checking') : 'idle'); }}><ICN.Git size={14} /> GitHub</button>
        <button className={activeMode === 'zip' ? 'active' : ''} onClick={() => { setActiveMode('zip'); setImportPhase(zipFile ? 'zip_ready' : 'idle'); }}><ICN.Box size={14} /> ZIP upload</button>
      </div>
        ...
        <div className="bld-split">
          <div className="github-pull-toggle">
            <div className="github-pull-head">
              <div className="github-pull-icon">{activeMode === 'zip' ? <ICN.Box size={18} /> : <ICN.Github size={18} />}</div>
              <div><div className="eyebrow">{activeMode === 'zip' ? 'ZIP upload' : 'GitHub pull'}</div><h2>{activeMode === 'zip' ? 'Drag and drop to deploy' : 'Import from repository'}</h2></div>
            </div>
            ...
            {activeMode === 'github' ? (
              <div className="builder-import-pane">
                ...
                <div className="input-group">
                  <input autoFocus className="input mono" placeholder="https://github.com/your-org/your-site" value={repoUrl} onChange={(e) => updateRepoUrl(e.target.value)} onPaste={(e) => { const pasted = e.clipboardData?.getData('text'); if (pasted) { e.preventDefault(); updateRepoUrl(pasted); } }} onKeyDown={(e) => e.key === 'Enter' && handleGitConnect()} />
                  <button className="btn btn-primary" onClick={handleGitConnect} disabled={gitBusy || importPhase === 'complete' || !detectedRepo}><ICN.Git size={14} /> {importPhase === 'complete' ? 'Opening' : gitBusy ? 'Importing' : 'Import'}</button>
                </div>
                <div style={{ marginTop: 12 }}><div className="label">Branch</div><input className="input mono" placeholder="main" value={repoBranch} onChange={(e) => setRepoBranch(e.target.value)} /></div>
              ...
              <div className="builder-import-pane">
                <div
                  ...
                  style={{ border: `2px dashed ${dragging ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 'var(--r-lg)', padding: 28, textAlign: 'center', background: dragging ? 'var(--accent-soft)' : 'var(--bg-deep)', cursor: 'pointer' }}
                >
                  <input ref={fileInputRef} type="file" accept=".zip,application/zip" style={{ display: 'none' }} onChange={(e) => selectZip(e.target.files?.[0])} />
                  <div style={{ width: 52, height: 52, borderRadius: 999, background: 'var(--accent-soft)', color: 'var(--accent)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}><ICN.Box size={24} /></div>
                  <h3 style={{ margin: '0 0 6px' }}>{zipFile ? zipFile.name : 'Drop your website ZIP here'}</h3>
                  <p className="muted" style={{ margin: 0, fontSize: 13 }}>ZIP must contain a deployable site with package.json or index.html. Node modules and .git folders are ignored.</p>
                </div>
                {zipError && <div style={{ marginTop: 10, color: 'var(--danger)', fontSize: 13 }}>{zipError}</div>}
                <button className="btn btn-primary" style={{ width: '100%', marginTop: 12 }} onClick={handleZipDeploy} disabled={zipBusy || !zipFile}><ICN.Rocket size={14} /> {zipBusy ? 'Uploading...' : 'Deploy ZIP'}</button>
              </div>
            )}

            <div className="render-config-panel">
              <div><div className="eyebrow">Render deployment payload</div><h3>Service settings</h3></div>
              ...
              <div><h3>{isStaticSite ? 'Static site settings' : 'Web service settings'}</h3></div>
              <div className="render-config-grid">
                <label><span>{isStaticSite ? 'Root directory' : 'Service root'}</span><input className="input mono" value={isStaticSite ? renderConfig.frontendRootDirectory : renderConfig.backendRootDirectory} onChange={(e) => updateRenderConfig(isStaticSite ? 'frontendRootDirectory' : 'backendRootDirectory', e.target.value)} placeholder={isStaticSite ? './' : 'server'} /></label>
                ...
                {isStaticSite ? <label><span>Publish directory</span><input className="input mono" value={renderConfig.frontendPublishDirectory} onChange={(e) => updateRenderConfig('frontendPublishDirectory', e.target.value)} /></label> : <label><span>Start command</span><input className="input mono" value={renderConfig.backendStartCommand} onChange={(e) => updateRenderConfig('backendStartCommand', e.target.value)} /></label>}
              </div>
              {activeMode === 'zip' && <div style={{ marginTop: 12 }}><div className="label">Optional generated-sites repository</div><input className="input mono" value={renderConfig.repoUrl} onChange={(e) => updateRenderConfig('repoUrl', e.target.value)} placeholder="https://github.com/your-org/generated-sites" /><div className="muted" style={{ fontSize: 12, marginTop: 6 }}>Leave blank to use RENDER_GENERATED_SITES_REPO_URL.</div></div>}
            </div>
          </div>
          <div className="bld-preview"><ImportProgressPreview phase={importPhase} repo={detectedRepo} branch={repoBranch || 'main'} error={activeError} showLoader={importStarted} isImporting={isImporting} zipFile={activeMode === 'zip' ? zipFile : null} /></div>
        </div>
      </div>
    </>
  );
