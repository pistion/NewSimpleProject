 ...
 *
 * Normalizes template selection metadata and reads lightweight catalog data
 * from the configured template library repository.
 */
...
export function selectTemplate(input = {}) {
  ...
  return {
    templateId,
    templateRepoUrl: input.templateRepoUrl || getTemplateLibraryConfig().repoUrl,
    templatePath: input.templatePath || templatePathForId(templateId),
...
export function normalizeTemplateId(value) {
  ...
  if (!id) return '';
  if (id.length > 100) throw badRequest('templateId is too long.', 'template_selection', 'TEMPLATE_ID_TOO_LONG');
  if (!/^[a-z0-9][a-z0-9._-]*$/i.test(id)) {
    throw badRequest('templateId contains unsupported characters.', 'template_selection', 'TEMPLATE_ID_INVALID');
  }
  return id;
}
...
export async function getTemplateRepositoryFiles(templateId) {
  const id = normalizeTemplateId(templateId);
  const templatePath = templatePathForId(id);
  const config = getTemplateLibraryConfig();
...
export async function readTemplateRepositoryFile(repoPath) {
