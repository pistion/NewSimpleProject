...
// ─── autofillBrief ────────────────────────────────────────────────────────────
/**
 * Suggest values for empty optional brief fields using the business name + industry
 * as anchors. Only fills fields that are currently blank.
 * @param {string} planId
 * @returns {Promise<{ suggestions: object, summary: string }>}
 */
export async function autofillBrief(planId) {
  ...
  // Only send truly empty fields so AI knows what's needed
  const emptyFields = [
...
Return strict JSON only:
{
  ...
  // Strip out any suggestions for fields that already had values
  const safe = {};
...
/**
 * Suggest an improved/enriched sections list for a specific page.
 * Returns sections with `contentHints` added.
 * @param {string} planId
 * @param {string} pageId
 * @returns {Promise<{ pageId: string, sections: object[], summary: string }>}
 */
...
// ─── suggestWireframe ─────────────────────────────────────────────────────────
/**
 * Add contentHints and layoutNotes to every section across all pages —
 * giving the wireframe view meaningful guidance without changing the structure.
 * @param {string} planId
 * @returns {Promise<{ pages: object[], summary: string }>}
 */
...
Return strict JSON only:
{
  ...
  // Merge AI hints back onto existing pages, preserving structure
