...
async function deploySite(req, res, next) {
  try {
    ...
    const {
      ...
      buildCommand = 'npm install && npm run build', publishDirectory = 'dist', sourceReference = 'roxanne-ai-tailored-template',
    ...
    const renderSourceRepo = repoUrl || repositoryUrl || process.env.RENDER_GENERATED_SITES_REPO_URL || '';
    ...
    if (!renderSourceRepo) {
      render.skippedReason = 'No Render source repository configured. Set RENDER_GENERATED_SITES_REPO_URL or send repoUrl in the deploy request.';
      ...
      try {
        ...
        const serviceResponse = await renderApiService.createService({
          ...
          serviceType,
          plan,
          repoUrl: renderSourceRepo,
          branch,
          rootDirectory: renderRootDirectory,
          buildCommand,
          outputDirectory: publishDirectory,
          sourceReference: renderSourceRepo,
        });
        ...
        currentStep = 'Generated locally; Render handoff failed';
    ...
    await updateTemplateSite(siteId, {
      ...
      generatedSite,
      render,
      deploymentSettings: { siteName: finalSiteName, slug: finalSlug, serviceType, plan, environment, buildCommand, publishDirectory, repoUrl: renderSourceRepo || null, rootDirectory: renderRootDirectory || null },
    });

    res.json({ status, siteId, deploymentId, templateId: site.templateId, generatedSite, render, liveUrl, message: render.attempted && !errorMessage ? 'Generated site and started Render deployment.' : 'Generated site and created Hosting record. Render handoff needs source repository/API configuration.' });
  } catch (err) { next(err); }
}
