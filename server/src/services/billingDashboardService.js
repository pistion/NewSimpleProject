/**
 * billingDashboardService.js
 *
 * Read-model aggregation for the user-facing Billing dashboard.
 * Combines data from the JSON hosting store, Prisma checkout orders, and
 * promo service into a single shape that the controller returns as-is.
 *
 * Keeping this out of billing.controller.js keeps the controller thin and
 * makes the data-gathering logic unit-testable without HTTP concerns.
 */
...
import { getPromoUsage, getUserPromoStatus } from './deploymentPromoService.js';
...
/**
 * Build the full billing dashboard payload for a given user.
 * @param {string|null} userId
 * @returns {Promise<{ pricing, orders, deployments, provider }>}
 */
export async function getUserBillingSummary(userId) {
  ...
  // 2. Their deployment orders — tied to owned deployments (robust ownership)
  //    and, as fallback, orders carrying their userId.
