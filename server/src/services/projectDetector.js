...
export async function detectProject(siteDir, files = []) {
  ...
  if (pkg) {
    ...
    if (scripts.start || set.has('server.js') || set.has('app.js') || set.has('src/server.js')) return preset('node-server', 'Node.js server', 'web_service', 'npm install', '.', scripts.start ? 'npm start' : 'node server.js', 'node', nodeVersion);
    return preset('node-source', 'Node static app', 'static_site', 'npm run build', 'dist', null, null, nodeVersion);
  }
  ...
  return preset('unknown', 'Unknown', 'static_site', 'npm run build', 'dist');
}
