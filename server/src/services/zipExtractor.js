...
const SKIP_FOLDERS = [
  ...
  'coverage/', '.cache/', '.parcel-cache/', '.turbo/', '.vite/', '__MACOSX/',
  '.idea/', '.vscode/', '.pnpm-store/', '.yarn/cache/',
];
...
export async function extractZipSafely(zipBuffer, destination, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (entry.header.size > maxEntryBytes) {
      throw badRequest(`ZIP entry is too large: ${entry.entryName}. Max per file is ${maxEntryBytes} bytes.`, 'ZIP_ENTRY_TOO_LARGE', 'zip_validation');
    }

    if (files.length >= maxFiles) {
      throw badRequest(`ZIP has too many deployable files after cleanup. Max: ${maxFiles}.`, 'ZIP_TOO_MANY_DEPLOYABLE_FILES', 'zip_validation');
    }
  ...
  if (!hasDeployableRoot(files)) {
    throw badRequest('ZIP does not look deployable. Expected package.json, index.html, dist/index.html, build/index.html, out/index.html, or public/index.html.', 'ZIP_NOT_DEPLOYABLE', 'project_detection');
  }
...
function hasDeployableRoot(files) {
  ...
  return set.has('package.json') || set.has('index.html') || set.has('dist/index.html') ||
    set.has('build/index.html') || set.has('out/index.html') || set.has('public/index.html');
}
