...
export async function publishDirectoryToGithub({ directory, targetRoot, repoUrl, branch = 'main', token }) {
  ...
  if (!parsed) stageError('Invalid generated-sites GitHub repository URL.', 'github_repo_validate', 400);
  if (!hasRealValue(token)) stageError('GITHUB_GENERATED_SITES_TOKEN or GITHUB_TOKEN is required to publish ZIP source files.', 'github_push', 409);

  const files = await walkFiles(directory);
  if (!files.length) stageError('No files found to publish to GitHub.', 'github_push', 400);
...
async function upsertGithubFile({ owner, repo, path: targetPath, branch, token, content, message }) {
  ...
  if (existing.ok) {
    ...
    throw new Error(`GitHub lookup failed ${existing.status}: ${body}`);
  }
  ...
  if (!put.ok) {
    ...
    throw new Error(`GitHub publish failed ${put.status}: ${body}`);
  }
...
function githubHeaders(token) {
  return {
    ...
    'User-Agent': 'glondiasites-render-deploy',
  };
}
